from flask import Blueprint, render_template, url_for, request, jsonify, redirect, session, current_app
...
def init_temperature():
    ...
    print(f"Available sensors: {control_bp.paths}")
    time.sleep(5)
    ...
    for temp_idx in range(control_bp.pixel):
        try:
            ...
            sensors[temp_idx] = 1
            print(f"Initialized sensor {temp_idx}: {sensor}")
            time.sleep(5)
        except Exception as e:
            print(f"Exception {e} in init_temperature for sensor {temp_idx}")
            time.sleep(5)
...
def ajax_response():
    ...
    try:
        ...
        print("Temperature values retrieved successfully")
        time.sleep(5)
        return jsonify(data={"times": times, "temps": temps, "paths": paths, "Exception": None}, success=True)
    
    except Exception as e:
        print(f"Exception in ajax_response: {e}")
        time.sleep(5)
...
def set_voltage():
    ...
    try:
        ...
        print(f"Set voltage: {set_voltage}, ADC voltage: {adc_volt}")
        time.sleep(5)
        return jsonify(data={"voltage": f"{set_voltage:.4f}", "adc_voltage": f"{adc_volt:.3f}", "Exception": None}, success=True)
    except Exception as e:
        print(f"Exception {e} in set_voltage")
        time.sleep(5)
...
def get_single_voltage():
    print("Getting single ADC value")
    time.sleep(5)
    ...
    print(f"Single ADC value: {voltage}")
    time.sleep(5)
...
def get_voltage():
    ...
    volt_card_idx = float(request.json["voltage_card"])
    print(f"Voltage card index: {volt_card_idx}")
    time.sleep(5)
    ...
    try:
        ...
        print(f"Exception in get_voltage: {e}")
        time.sleep(5)
...
def MakeMonitorPlotForBias(logfile):
    print("Making monitor plot for bias")
    time.sleep(5)
